...
object HiddenModuleStore {
    ...
    fun loadSnapshot(@Suppress("UNUSED_PARAMETER") context: Context): HiddenModuleSnapshot {
        ...
        return runCatching {
            val state = JSONObject(payload ?: throw IllegalStateException("Missing state payload"))
            if (state.optInt("version", -1) != PAYLOAD_VERSION) {
                throw IllegalStateException("Unsupported state payload version")
            }
            val array = state.optJSONArray("hidden_ids")
                ?: throw IllegalStateException("Missing hidden module list")
            ...
            HiddenModuleSnapshot(
                ...
                shouldHideAllModules = !hasPassword,
    ...
    fun verifyPassword(@Suppress("UNUSED_PARAMETER") context: Context, password: String): Boolean {
        ...
        return runCatching {
            ...
            if (json.optInt("version", -1) != PAYLOAD_VERSION) {
                return false
            }
