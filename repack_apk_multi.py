...
MANAGER_BUILD_VERSION_CODE = 2_100_000_000
MANAGER_VERSION_CODE = 2_200_510_202
...
def patch_manifest_version_code(
    ...
    """Patch the binary manifest versionCode to the requested unsigned 32-bit value."""
    ...
    with ZipFile(apk_path, "r") as source_apk:
        ...
        if patched_manifest.count(new_value) != 1:
            raise RuntimeError(
                "Patched binary manifest versionCode verification failed, "
                f"found {patched_manifest.count(new_value)} occurrences for {requested_version_code}"
            )
    ...
    with ZipFile(patched_path, "r") as patched_apk:
        read_back_manifest = patched_apk.read("AndroidManifest.xml")
    read_back_count = read_back_manifest.count(new_value)
    if read_back_count != 1:
        patched_path.unlink(missing_ok=True)
        raise RuntimeError(
            "Read-back binary manifest versionCode verification failed, "
            f"found {read_back_count} occurrences for {requested_version_code}"
        )
    ...
    print(
        ...
        f"{requested_version_code} (0x{requested_version_code:08x}, "
        f"signed minor {signed_minor}, raw uint32 {signed_minor & 0xffffffff})"
    )
